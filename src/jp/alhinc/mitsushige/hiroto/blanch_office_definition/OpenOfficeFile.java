	...
	public static void main(String[] args) throws IOException {
		...
		try{
			...
			if(CheckFile(file)){
				...
				while((str=br.readLine())!=null){
					...
					if(data[0].matches("^\\d{3}")&&(data[1].matches("\\S+"))&&data.length==2){
						System.out.println(str);
					}else{
						System.out.println("支店定義ファイルのフォーマットが不正です。");
						return;
					}
					branch.put(data[0],data[1]);
				}
			}else{
		...
		try{
			...
			if(CheckFile(file)){
				...
				while((str=br.readLine())!=null){
					...
					//フォーマットチェック
					if(data[0].matches("^\\w{8}")&&(data[1].matches("\\S+"))&&data.length==2){
						System.out.println(str);
					}else{
						System.out.println("支店定義ファイルのフォーマットが不正です。");
						return;
					}
					commodity.put(data[0],data[1]);
				}
			}else{
				System.out.println("ファイルを読み込めません。。");
			}
			...
			try{
				for(int i = 0; i<rcdList.size(); i++){
					...
					commodityMap.put(rcd.get(1),Sale);
		...
		try{
			...
			for (Entry<String,Long> o : entries) {
				...
				bw.write(o.getKey()+","+branch.get(o.getKey())+","+o.getValue());
				bw.newLine();
				System.out.println(o.getKey()+","+branch.get(o.getKey())+","+o.getValue());
		...
		try{
			...
			List<Map.Entry<String,Long>> entrie =
					new ArrayList<Map.Entry<String,Long>>(commodityMap.entrySet());
			Collections.sort(entrie, new Comparator<Map.Entry<String,Long>>() {
			...
			for (Entry<String,Long> o : entrie) {
				bw.write(o.getKey()+","+commodity.get(o.getKey())+","+(o.getValue()));
				bw.newLine();
				System.out.println(o.getKey()+","+commodity.get(o.getKey())+","+(o.getValue()));
			}
