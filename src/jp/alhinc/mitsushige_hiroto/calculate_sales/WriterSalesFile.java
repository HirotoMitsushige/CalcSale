...
public class WriterSalesFile{
	...
	public static void main(String[] args) {
		...
		try{
			...
			for(int i = 0; i < fileList.length; i++){
				if(checkFile(new File(args[0], fileList[i])) && fileList[i].matches("\\d{8}.rcd$")){
	...
	//支店、商品定義ファイル------------------------------------------------------------------------------
	public static boolean readSaleFile(String dirPath, String fileLst, String expression, String errMessage,
		...
		try{
			...
			while((str = br.readLine()) != null){
				...
				if(!(data.length == 2 && data[0].matches(expression))){
	...
	public static boolean writerFile(String dirPath, String fileName,
			HashMap<String, Long> salesMap, HashMap<String, String> nameMap){
